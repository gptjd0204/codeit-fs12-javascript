...
function renderEntry(entry) {
  ...
  btnContainer.className = "entry-actions";
  ...
  entryContainer.appendChild(btnContainer);
...
function renderAllEntries(mood) {
  ...
  diaryList.innerHTML = "";
  if (diary.length === 0) {
    diaryList.previousElementSibling.textContent = `일기 목록`;
...
function updateEntry(id, updates) {
  ...
  const findEntry = diary.find((entry) => {
    return id === entry.id;
  });
  ...
  if (findEntry.length === 0) {
    ...
    Object.assign(findEntry, updatedEntry);
  }
}
...
function fillFormForEdit(id) {
  ...
  if (findEntry.length === 0) {
    ...
    titleInput.value = findEntry.title;
    contentInput.value = findEntry.content;
    moodSelect.value = findEntry.mood;
...
function deleteEntry(id) {
  ...
  const findIndexEntry = diary.findIndex((entry) => {
    return id === entry.id;
  });
...
function getDiaryStats() {
  ...
  diary.forEach((entry) => {
    if (diaryStat.moods[entry.mood]) {
      diaryStat.moods[entry.mood]++;
    } else {
      diaryStat.moods[entry.mood] = 1;
    }
  });
...
function renderStats() {
  ...
  if (stats.length === 0 || stats.total === 0) {
  ...
  const diaryTotal = document.createElement("p");
  diaryTotal.className = "stat-total";
  diaryTotal.textContent = `전체 일기: ${stats.total}개`;
  statsContainer.appendChild(diaryTotal);

  for (const mood in stats.moods) {
    const moodsTotal = document.createElement("span");
    moodsTotal.className = "stat-item";
    let moodTrans;
    switch (mood) {
      case "happy":
        moodTrans = "행복";
    ...
    moodsTotal.textContent += `${MOOD_EMOJIS[mood]} ${moodTrans} : ${stats.moods[mood]} `;
    statsContainer.appendChild(moodsTotal);
  }
}
...
// 도전 과제 (다 풀었다면 시도해보세요!)
// ============================================
// 1. getEntriesByKeyword(keyword) [진행중: 함수만 작성]
//    - 제목이나 내용에 keyword가 포함된 일기를 찾아 반환
//    - 힌트: filter() + includes()
function getEntriesByKeyword(keyword) {
  diary.filter((entry) => {
    return entry.includes(keyword);
  });
}
...
// 3. 일기 개수를 헤더 제목 옆에 실시간으로 표시하기 [완]
//    - 예: "일기 목록 (3)"
